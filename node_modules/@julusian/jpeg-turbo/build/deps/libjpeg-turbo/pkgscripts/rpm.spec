...
%define _prefix /opt/jpegturbo
%define _bindir /opt/jpegturbo/bin
%define _datarootdir /opt/jpegturbo
%define _includedir /opt/jpegturbo/include
%define _javadir 
%define _mandir /opt/jpegturbo/man
...
%endif
#-->%else
%define _libdir /opt/jpegturbo/lib64
#-->%endif

Summary: A SIMD-accelerated JPEG codec that provides both the libjpeg and TurboJPEG APIs
Name: jpegturbo
Version: 2.0.4
Vendor: The jpegturbo Project
URL: http://www.jpegturbo.org
Group: System Environment/Libraries
#-->Source0: http://prdownloads.sourceforge.net/jpegturbo/jpegturbo-%{version}.tar.gz
Release: 20230312
License: BSD-style
BuildRoot: %{_blddir}/%{name}-buildroot-%{version}-%{release}
Requires: /sbin/ldconfig
%if "%{_bits}" == "64"
Provides: %{name} = %{version}-%{release}, jpegturbo = %{version}-%{release}, libturbojpeg.so()(64bit)
%else
Provides: %{name} = %{version}-%{release}, jpegturbo = %{version}-%{release}, libturbojpeg.so
%endif
...
LJT_DOCDIR=/opt/jpegturbo/doc
if [ "%{_prefix}" = "/opt/jpegturbo" -a "$LJT_DOCDIR" = "/opt/jpegturbo/doc" ]; then
...
%doc %{_docdir}/*
%dir %{_prefix}
%if "%{_prefix}" == "/opt/jpegturbo" && "%{_docdir}" != "%{_prefix}/doc"
 %{_prefix}/doc
%endif
%dir %{_bindir}
%{_bindir}/cjpeg
